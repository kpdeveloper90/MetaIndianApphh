...
public class TrackingController implements PositionProvider.PositionListener, NetworkManager.NetworkHandler {
    ...
    public TrackingController(Context context) {
        ...
        url = preferences.getString(AppConfig.KEY_URL, Connection.Traccar_url_value);
    ...
    public void onPositionUpdate(Position position) {
        Log.e(TAG, context.getString(R.string.status_location_update));
    ...
    private void log(String action, Position position) {
        if (position != null) {
            action += " (" +
                    ...
                    " lon:" + position.getLongitude() + ") \t " + Connection.getCurrentDateTime12Hours();
        }
        Log.e(TAG, action);
    }
    ...
    private void read() {
        ...
        databaseHelper.selectPositionAsync(new DatabaseHelper.DatabaseHandler<Position>() {
            ...
            public void onComplete(boolean success, Position result) {
                if (success) {
                    if (result != null) {
                        if (result.getDeviceId().equals(preferences.getString(AppConfig.KEY_DEVICE, null))) {
                            send(result);
                        } else {
                            delete(result);
                        }
        ...
        RequestManager.sendRequestAsync(request, new RequestManager.RequestHandler() {
            ...
            public void onComplete(boolean success) {
                if (success) {
                    ...
                    Log.e(TAG, context.getString(R.string.status_send_fail));
