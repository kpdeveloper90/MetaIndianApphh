 ...
 */
public class DriverComplaints extends Fragment implements View.OnClickListener, SearchView.OnQueryTextListener, SwipeRefreshLayout.OnRefreshListener, ConnectionReceiver.ConnectionReceiverListener {
    private static final String TAG = DriverComplaints.class.getSimpleName();
    ...
    @BindView(R.id.srl_driver_Complaints)
    SwipeRefreshLayout srl_driver_Complaints;

    @BindView(R.id.rv_driver_Complaints)
    RecyclerView rv_driver_Complaints;
...
//        ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Delivery Counter");
        ...
        rv_driver_Complaints.setHasFixedSize(true);
        rv_driver_Complaints.setLayoutManager(lLayout);
        rv_driver_Complaints.setItemAnimator(new DefaultItemAnimator());
        compReaddressedAdapter = new CompReaddressedAdapter(getActivity(), complaintsList, TAG);

        rv_driver_Complaints.setAdapter(compReaddressedAdapter);
        rv_driver_Complaints.addOnItemTouchListener(new RecyclerItemClickListener(getActivity(), rv_driver_Complaints, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                ...
                mIntent.setAction("AssignDriverComplaints");
        ...
        rv_driver_Complaints.addOnScrollListener(new RecyclerView.OnScrollListener() {
...
//        sp_assets_wardNumber.setAdapter(adapter_wardNo);
        ...
        srl_driver_Complaints.setOnRefreshListener(this);
        srl_driver_Complaints.setColorSchemeResources(R.color.colorAccent);
        srl_driver_Complaints.setNestedScrollingEnabled(true);
        srl_driver_Complaints.post(
                new Runnable() {
                    @Override
                    public void run() {
...
//                        getComplaintsDetailOnServer(0);
    ...
    public void onRefresh() {
        srl_driver_Complaints.setRefreshing(true);
...
//            String wardNo = sp_assets_wardNumber.getSelectedItem().toString();
        ...
        srl_driver_Complaints.setRefreshing(false);
    }
...
//                            + "\"]]]";
                    ...
                    if (CptStatus.equalsIgnoreCase("NA")) {
                        filters = filters + "[\"Complaints\",\"cptstatus\",\"in\",[\"Noted\"]]";
